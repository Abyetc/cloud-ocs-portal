...
public class SyncCityStateServiceImpl implements SyncCityStateService {
	...
	private void checkCityState(City city) {
		...
		List<CityNetwork> allCityNetworkList = cityNetworkDao
				.findCityNetworksByCityId(city.getId());
		if (allCityNetworkList != null) {
			...
			for (CityNetwork cityNetwork : allCityNetworkList) {
				...
				if (cityNetwork.getNetworkState() == NetworkState.IMPLEMENTED
						.getCode()
						&& ocsVmService.getOcsVmsNumInInLBRule(networkId) > 0) {
